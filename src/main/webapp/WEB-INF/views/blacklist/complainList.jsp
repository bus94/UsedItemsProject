	...
	<div
		...
		<div id="blacklist_title_container"
			...
			<p id="main_title" class="fs-1">
				<span id="safeMarket">Safe Market</span>에
				<span id="singo">신고</span>가
				등록되었어요
			</p>
		...
		<!-- 로그인 시 '나의 신고내역'과 '신고하러 가기' 버튼 보이기 -->
		<c:if test="${loginMember!=null}">
			<div class="btn_container container d-flex mb-1">
				<form action="${path}/blacklist/complainList.do" method="get">
					...
					<button type="submit" class="btn btn-warning btn-sm mb-3">나의 신고내역</button>
				</form>
				<button type="button" class="btn btn-danger btn-sm mb-3"
					onclick="location.href='${path}/blacklist/complain.do'">
					신고하러 가기</button>
			</div>
		</c:if>
		...
		<table id="blacklist_container" class="table table-bordered">
			...
			<tbody>
				<c:if test="${empty blacklist}">
					<tr>
						<td colspan="5" id="empty_td" class="text-center">
						조회된 글이 없습니다.</td>
					</tr>
				</c:if>
				<c:if test="${not empty blacklist}">
					<c:forEach var="each" items="${blacklist}">
						<tr class="text-center">
							...
							<td class="list_content text-start">
								<!-- 관리자 또는 본인 작성글인 경우 삭제 버튼 보이기 -->
								<c:if test="${loginMember.acc_id==each.subject_id||loginMember.acc_status=='admin'}">
									<a href="complainDel.do?black_index=${each.black_index}"
										id="del_black">&lt;삭제&gt;&nbsp;</a>
								</c:if> <span>${each.black_content}</span>
							</td>
